...
## 5.5 테스트 실행 — 반드시 `gapt_test` DB 로

DSN-gated 통합 테스트 (`server/tests/**/test_routes.py` 등) 는 매
실행마다 `DROP SCHEMA public CASCADE` 후 alembic upgrade 합니다.
**`GAPT_TEST_POSTGRES_DSN` 을 live `gapt` DB 로 가리키면 운영
데이터 (프로젝트 / 환경 / secret / Cloudflare 설정) 가 매번
초기화됩니다.** 2026-05-28 에 실제로 한 번 일어났음.

```bash
# 권장 (전용 test DB):
GAPT_TEST_POSTGRES_DSN="postgresql://gapt:gapt_dev_only@127.0.0.1:35432/gapt_test" \
  uv run pytest
```

`gapt_test` 는 fresh compose-up 시 `compose/postgres/initdb/01-gapt-test.sql`
가 자동 생성. 기존 볼륨에 없으면:
...
가드 (`server/tests/_helpers/db_guard.py`) 가 DB 이름이 `_test`
suffix / `test_` prefix / literal `test` 중 하나가 아니면 거부함 —
실수로 `…/gapt` 를 export 해도 DROP 이 실행되기 전에
`UnsafeTestDsnError` 로 막힙니다.
